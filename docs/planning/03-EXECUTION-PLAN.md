...
|---|---|---|---|
| T4-PARCEL-GRAPH | Codex | 2026-04-21 | Parcel summary backend plus parcel detail browser proof for history, vistorias, and IPTU are covered; cross-module graph still incomplete. |
...
1. **T2-PARCEL-E2E** — DONE (search → detail → edit → persist verified; `T4-PARCEL-GRAPH` now also has parcel summary and parcel detail browser proof for history, vistorias, and IPTU).
...
7. **T3-DASH-PROOF** — PARTIAL (layout persistence plus `/dashboard/kpis`, `/dashboard/executive`, visible KPI cards, backend contract coverage in `DashboardService`, and filtered dashboard coverage in `MonitoringService` are proved; broader observability coverage still pending).
