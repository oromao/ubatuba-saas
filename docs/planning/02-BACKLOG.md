...
- **Agente:** Codex (2026-04-21) — layout do dashboard persiste em reload, `/dashboard/kpis` e `/dashboard/executive` estão provados, os cards de KPI foram ligados ao payload real, os sinais de prontidão/satélites seguem auditáveis, e `DashboardService` agora também tem prova unitária do contrato executivo; ainda falta ampliar KPIs/observabilidade satélite.
