...
- **Agente:** Codex (2026-04-21) — prova de dataset >10k e fallback explícito validada; o helper de bounds é compartilhado entre mapa e teste, mas o render real do mapa em escala continua dependente do ambiente WebGL do runner.
...
- **Agente:** Codex (2026-04-21) — `assets`, `logradouros`, `pgv/zonas`, `pgv/faces`, `ctm/mobiliario`, `ctm/parcelas`, `156`, `ctm/vistorias`, `ambiental`, `levantamentos`, `modulos/compliance`, `cartas`, `pgv/relatorio`, `integracoes`, `reurb` e `monitoramento` agora mostram error/empty state explícitos com fallback testado; ainda faltam outros módulos do padrão.
...
- **Agente:** Codex (2026-04-21) — layout do dashboard persiste em reload, `/dashboard/kpis` e `/dashboard/executive` estão provados, os cards de KPI foram ligados ao payload real, e os sinais de prontidão/satélites seguem auditáveis; ainda falta ampliar KPIs/observabilidade satélite.
