...
const monitoring_service_1 = require("../src/modules/monitoring/monitoring.service");
...
describe('MonitoringService', () => {
    it('ingests environmental event and creates alert', async () => {
        const service = new monitoring_service_1.MonitoringService(repository, alertsService, cache);
    ...
    it('keeps dashboard aggregation stable when filters are applied', async () => {
        ...
        const service = new monitoring_service_1.MonitoringService(filteredRepository, alertsService, cache);
        const dashboard = await service.dashboard('66f1f77a67e30f9f62000001', {
            stage: 'TRIAGEM',
            sourceMode: 'API',
        });
        expect(filteredRepository.list).toHaveBeenCalledWith('66f1f77a67e30f9f62000001', expect.objectContaining({
            stage: 'TRIAGEM',
            sourceMode: 'API',
        }));
    ...
    it('promotes event to evidence stage when attaching evidence without explicit stage', async () => {
        ...
        const service = new monitoring_service_1.MonitoringService(repo, alertsService, cache);
    ...
    it('blocks invalid stage transition', async () => {
        ...
        const service = new monitoring_service_1.MonitoringService(repo, alertsService, cache);
        await expect(service.triage('66f1f77a67e30f9f62000001', 'evt-1', { message: 'Nao deve reabrir' })).rejects.toBeInstanceOf(common_1.BadRequestException);
    });
});
//# sourceMappingURL=monitoring.service.spec.js.map