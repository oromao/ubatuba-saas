...
const observatory_service_1 = require("../src/modules/observatory/observatory.service");
describe('ObservatoryService', () => {
    ...
    const projectsService = { resolveProjectId: jest.fn().mockResolvedValue('proj-1') };
    const cacheService = { get: jest.fn().mockResolvedValue(null), set: jest.fn() };
    const service = new observatory_service_1.ObservatoryService(parcelsService, valuationsService, monitoringService, projectsService, cacheService);
    it('builds comparative market overview', async () => {
        const result = (await service.marketOverview('tenant', 'proj-1', 'arrecadacao'));
    ...
    it('builds zone comparative breakdown when compare=zone', async () => {
        ...
        expect(result.comparativeBreakdown.some((item) => item.label === 'Z1')).toBe(true);
    });
...
//# sourceMappingURL=observatory.service.spec.js.map