...
describe('T7-SP-DATA-REAL: GeoJSON import with dirty data', () => {
    ...
    let tenantId;
    let accessToken;
    const dirtyFixture = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../../../../test/fixtures/sp-dirty-data-test.geojson'), 'utf-8'));
    beforeAll(async () => {
    ...
    describe('Import with dirty SP data', () => {
        it('imports valid Polygon and MultiPolygon features', async () => {
            const validFeatures = {
                ...
                features: dirtyFixture.features.filter((f) => f.geometry && (f.geometry.type === 'Polygon' || f.geometry.type === 'MultiPolygon')),
            };
            ...
            expect(response.body.inserted).toBeGreaterThanOrEqual(2);
...
//# sourceMappingURL=parcels-import-dirty.spec.js.map