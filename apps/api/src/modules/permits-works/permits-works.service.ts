...
import { PermitWorkRequestDocument, PermitWorkStage, PermitWorkStatus } from './permit-work.schema';

const WORK_STAGE_TO_STATUS: Record<PermitWorkStage, PermitWorkStatus> = {
  ABERTURA: 'ABERTO',
...
const WORK_TRANSITIONS: Record<PermitWorkStage, PermitWorkStage[]> = {
...
@Injectable()
export class PermitsWorksService {
  constructor(
    private readonly repository: PermitsWorksRepository,
  ...
  async create(tenantId: string, dto: CreatePermitWorkDto, actorId?: string) {
    const projectId = await this.projectsService.resolveProjectId(tenantId);
    const protocolNumber = `OB-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${randomUUID().slice(0, 6).toUpperCase()}`;
  ...
  async update(tenantId: string, id: string, dto: UpdatePermitWorkDto, actorId?: string) {
    ...
    if (dto.stage) {
      this.transition(request, dto.stage as PermitWorkStage, dto.message, actorId);
    } else if (dto.status) {
  ...
  async addInvoice(tenantId: string, id: string, description: string, amount: number) {
    const request = await this.repository.findById(tenantId, id);
    if (!request) throw new NotFoundException('Solicitacao nao encontrada');
  ...
  async addRequirementResponse(tenantId: string, id: string, requirementId: string, note: string, actorId?: string) {
    const request = await this.repository.findById(tenantId, id);
    if (!request) throw new NotFoundException('Solicitacao nao encontrada');
    const requirement = request.requirements.find((item) => item.id === requirementId);
    if (!requirement) throw new NotFoundException('Exigencia nao encontrada');
  ...
  async issueDecisionPdf(tenantId: string, id: string) {
    const request = await this.repository.findById(tenantId, id);
    if (!request) throw new NotFoundException('Solicitacao nao encontrada');
  ...
  private transition(request: PermitWorkRequestDocument, stage: PermitWorkStage, message?: string, actorId?: string, force = false) {
    ...
    if (stage === 'TAXAS') request.responsibleDepartment = 'Receita Municipal';
    if (stage === 'ASSINATURA' || stage === 'EMISSAO' || stage === 'ENCERRAMENTO') request.responsibleDepartment = 'Gabinete / Digital';
    request.history.push({
      id: randomUUID(),
      status: request.status,
      stage,
