      ...
      satelliteHealth: [
        {
          ...
          inProgress: calls.filter((item: any) => item.status === 'EM_TRIAGEM' || item.status === 'ENCAMINHADO' || item.status === 'EM_CAMPO').length,
          closed: calls.filter((item: any) => item.status === 'RESOLVIDO' || item.status === 'CANCELADO').length,
        },
        {
          id: 'environment',
          label: 'Ambiental',
          ...
          inProgress: environmentCases.filter((item: any) => item.status === 'EM_TRIAGEM' || item.status === 'EM_CAMPO').length,
          closed: environmentCases.filter((item: any) => item.status === 'RESOLVIDO' || item.status === 'LAUDO').length,
        },
        {
          id: 'public-works',
