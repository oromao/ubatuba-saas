...
export class EnvironmentService {
  ...
  async create(tenantId: string, dto: CreateEnvironmentCaseDto, actorId?: string) {
    const projectId = await this.projectsService.resolveProjectId(tenantId, dto.projectId);
    const protocolNumber = `AM-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${randomUUID().slice(0, 6).toUpperCase()}`;
  ...
  async update(tenantId: string, id: string, dto: UpdateEnvironmentCaseDto, actorId?: string) {
    const current = await this.repository.findById(tenantId, id);
    if (!current) throw new NotFoundException('Caso nao encontrado');
    if (dto.status) {
  ...
  async issueReport(tenantId: string, id: string) {
    const current = await this.repository.findById(tenantId, id);
    if (!current) throw new NotFoundException('Caso nao encontrado');
