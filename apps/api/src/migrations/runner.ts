...
import { calculateGeometryArea, PolygonGeometry } from '../common/utils/geo';
...
const UBATUBA_BBOX: [number, number, number, number] = [-45.110, -23.474, -45.040, -23.428];
...
const migrations: Migration[] = [
    ...
    run: async (connection) => {
      ...
      if (processCount === 0) {
        const items = Array.from({ length: 5 }).map((_, index) => ({
          tenantId,
          protocolNumber: `PR-${randomUUID().slice(0, 8).toUpperCase()}`,
  ...
  },
  {
    ...
    run: async (connection) => {
      ...
      await parcelSocioeconomic.createIndex(
        { tenantId: 1, projectId: 1, parcelId: 1 },
        { unique: true },
      );
      await parcelInfrastructure.createIndex(
        { tenantId: 1, projectId: 1, parcelId: 1 },
        { unique: true },
      );
      ...
      await pgvFactors.createIndex(
        { tenantId: 1, projectId: 1, category: 1, key: 1 },
        { unique: true },
      );
      ...
      await pgvAssessments.createIndex(
      ...
      if (parcelsCount === 0) {
        const parcelDocs: Array<Record<string, unknown>> = [];
        const cols = 5;
        const rows = 6;
      ...
      if (buildingCount === 0) {
        await parcelBuildings.insertMany(
      ...
      if (socioCount === 0) {
        await parcelSocioeconomic.insertMany(
          parcelIds.map((parcelId, index) => ({
            tenantId,
            projectId,
            parcelId,
      ...
      if (infraCount === 0) {
        await parcelInfrastructure.insertMany(
...
async function uploadMockGeotiff(key: string) {
  ...
  const buffer = await readFile(mockPath);
  await client.send(
...
async function publishGeoTiff({
  ...
  const upload = await geoserverRequest(
    `/rest/workspaces/${workspace}/coveragestores/${store}/file.geotiff?coverageName=${layerName}`,
    {
      method: 'PUT',
  ...
  await geoserverRequest(
    `/rest/workspaces/${workspace}/coveragestores/${store}/coverages/${layerName}.json`,
    {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
async function geoserverRequest(path: string, options: RequestInit) {
